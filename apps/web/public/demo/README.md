# Demo SEO before/after — Cà phê Arabica Cầu Đất (Highland Roastery)
...
Cùng một shop (**Highland Roastery**), cùng sản phẩm (**Cà phê Arabica Cầu Đất
rang xay 250g – 185.000đ**):

- `bad.html` — một trang shop **cơ bản, trông như thật** (header+nav, breadcrumb,
  gallery, giá, mô tả tự nhiên, thông số, đánh giá, footer) nhưng **không làm SEO**
  (thiếu meta/og/schema/alt/canonical/lang/viewport, dùng `<div>` thay `<h1>`, sót
  `noindex`). Kịch bản rất hay gặp ngoài thực tế.
- `good.html` — trang đẹp, chuẩn SEO. Mỗi sửa đổi **bám theo một gợi ý có thật**
  trong `evidence/bad-report.json` (xem bảng truy vết).

Cả hai trang **self-contained** (CSS + ảnh SVG inline, không phụ thuộc file ngoài).
...
|---|---|---|---|---|
| title_tag | Title length 17 is out of range | Add a title between 50 and 60 characters that includes the primary keyword. | `<title>` ~55 ký tự, chứa keyword + brand | ✅ |
| meta_description | Meta description is missing | Add a meta description between 120 and 160 characters. | `<meta name=description>` ~150 ký tự | ✅ |
...
| image_alt | Only 0% of images have alt text | Add descriptive alt attributes to improve accessibility and SEO. | `alt` mô tả + width/height + loading cho mọi ảnh | ✅ |
...
| schema_org | No structured data (JSON-LD) found | Add schema.org JSON-LD for Article, Product, FAQ, etc. | JSON-LD Product+Offer+AggregateRating + Breadcrumb | ✅ |
| language_tag | HTML lang attribute is missing | Add lang attribute to `<html>` element. | `<html lang="vi">` | ✅ |
| favicon | Favicon is missing | Add `<link rel="icon" href="/favicon.ico">`. | `<link rel="icon">` (favicon SVG inline) | ✅ |
...
1. **open_graph & twitter_card** — *bug của app, không phải của trang.*
   ...
   Bug đã được ghi nhận để sửa riêng (ngoài phạm vi demo này).
