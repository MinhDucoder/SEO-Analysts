...
describe('client.check', () => {
  it('sends Bearer header + JSON body and returns parsed JSON on 200', async () => {
    const fetchImpl = vi.fn().mockResolvedValue(
      fakeResponse({ ok: true, status: 200, body: { score: 91 } }),
    );
    const res = await check({ apiKey, baseUrl, body, fetchImpl });
  ...
  it('trims trailing slash from baseUrl so the path is well-formed', async () => {
    ...
    await check({ apiKey, baseUrl: 'http://localhost:3000/', body, fetchImpl });
  ...
  it('throws PublicApiError carrying the stable code from the envelope', async () => {
    ...
    await expect(check({ apiKey, baseUrl, body, fetchImpl })).rejects.toMatchObject({
