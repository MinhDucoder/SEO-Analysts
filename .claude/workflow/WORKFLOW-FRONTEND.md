...
| 3c | **FE↔BE integration** (L4 — real gateway + real DB) | Invoke skill [`fe-be-integration`](../skills/fe-be-integration/SKILL.md) for slugs touching auth, session, OAuth, rate-limit, CORS, or admin boundaries. Requires `npm run docker:up`. Catches wire-level bugs L3 mock cannot see (cookie HttpOnly, rate-limit, CORS preflight, OAuth state). | Large tier + slug touches auth/session/admin/OAuth/rate-limit. Skip for Small/Medium FE (L1-L3 đủ) |
...
- **Auth/session/admin/OAuth/rate-limit slug** → Gate 3c triggers skill [`fe-be-integration`](../skills/fe-be-integration/SKILL.md) to verify wire-level contract (cookie flags, CORS, rate-limit, OAuth state). Requires `npm run docker:up`. Runs opt-in locally via `npm run test:integration`; CI runs nightly via `.github/workflows/integration.yml`.
...
**Post-phase (L4 contract verify):** [`fe-be-integration`](../skills/fe-be-integration/SKILL.md) — invoked for auth/session/admin/OAuth slugs to test real gateway + real DB contract. Separate atomic commit. Covers wire-level bugs L1-L3 cannot see (cookie HttpOnly, rate-limit 429, CORS preflight, OAuth state). Nightly CI + manual trigger. See [SKILL.md](../skills/fe-be-integration/SKILL.md) for 9-step workflow.
